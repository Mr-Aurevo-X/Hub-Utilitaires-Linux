...
class AtelierPage:
    ...
    def _tab_text(self) -> Gtk.Widget:
        ...
        common.padded(box)
        self._pattern = common.t_entry("text_pattern")
        self._text_repl = common.t_entry("text_replace_with")
        self._ignore = common.t_check("text_ignore_case")
        ...
        diff = common.t_button("text_diff")
    ...
    def _tab_encode(self) -> Gtk.Widget:
        box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
        common.padded(box)
        hint = common.t_label("encode_hint", wrap=True, xalign=0)
    ...
    def _tab_generate(self) -> Gtk.Widget:
        ...
        box.append(uuid_row)
        self._uuid5_name = common.t_entry("gen_uuid5_name")
        ...
        now = common.t_button("gen_now")
        now.connect("clicked", self._now)
        to_iso = common.t_button("gen_unix")
        to_iso.connect("clicked", self._unix_to_iso)
        to_unix = common.t_button("gen_iso")
        to_unix.connect("clicked", self._iso_to_unix)
        plus = common.t_button("gen_plus_days")
        plus.connect("clicked", self._plus_days)
        week = common.t_button("gen_week")
        ...
        conv = common.t_button("gen_units")
        ...
        self._base_val = common.t_entry("gen_base")
        ...
        base_btn = common.t_button("gen_base_go")
        ...
        lorem_btn = common.t_button("gen_lorem")
        lorem_btn.connect("clicked", self._lorem)
        extra.append(lorem_btn)

        self._qr = common.t_entry("gen_qr_text")
        qr_btn = common.t_button("gen_qr")
        ...
        dummy_btn = common.t_button("gen_dummy")
        ...
        self._desktop_name = common.t_entry("gen_desktop_name")
        self._desktop_exec = common.t_entry("gen_desktop_exec")
        desktop_btn = common.t_button("gen_desktop")
        ...
        qr_read_btn = common.t_button("gen_qr_read")
        qr_read_btn.connect("clicked", lambda *_: compat.open_files(self._window, self._qr_read))
        extra.append(qr_read_btn)

        self._cron_expr = common.t_entry("gen_cron_expr")
        cron_btn = common.t_button("gen_cron")
        ...
        gitignore_btn = common.t_button("gen_gitignore")
    ...
    def _tab_password(self) -> Gtk.Widget:
        box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=8)
        common.padded(box)
        hint = common.t_label("atelier_password_hint", wrap=True, xalign=0)
        hint.add_css_class("dim-label")
        box.append(hint)
        ...
        common.bind_i18n(self._pwd_len, "gen_length", "tooltip")
        self._pwd_batch = Gtk.SpinButton.new_with_range(1, 50, 1)
        self._pwd_batch.set_value(1)
        common.bind_i18n(self._pwd_batch, "gen_batch", "tooltip")
        self._pwd_words = Gtk.SpinButton.new_with_range(3, 12, 1)
        self._pwd_words.set_value(4)
        common.bind_i18n(self._pwd_words, "gen_words", "tooltip")
        self._pin_len = Gtk.SpinButton.new_with_range(4, 12, 1)
        self._pin_len.set_value(6)
        common.bind_i18n(self._pin_len, "gen_pin_length", "tooltip")
        opts.append(common.t_label("gen_length"))
        opts.append(self._pwd_len)
        opts.append(common.t_label("gen_pin_length"))
        opts.append(self._pin_len)
        opts.append(common.t_label("gen_batch"))
        opts.append(self._pwd_batch)
        opts.append(common.t_label("gen_words"))
        opts.append(self._pwd_words)
        box.append(opts)

        classes = Gtk.Box(spacing=8)
        self._pwd_lower = common.t_check("gen_lower")
        self._pwd_lower.set_active(True)
        self._pwd_upper = common.t_check("gen_upper")
        self._pwd_upper.set_active(True)
        self._pwd_digits = common.t_check("gen_digits")
        self._pwd_digits.set_active(True)
        self._pwd_sym = common.t_check("gen_symbols")
        ...
        self._pwd_ambiguous = common.t_check("gen_exclude_ambiguous")
        self._pwd_ambiguous.set_active(True)
        self._pwd_ensure = common.t_check("gen_ensure")
    ...
    def _make_pin(self, *_args: object) -> None:
        length = int(self._pin_len.get_value())
        count = int(self._pwd_batch.get_value())
        pins = [generate.pin(length) for _ in range(count)]
        _set_buffer(self._pwd, "\n".join(pins))
        bits = generate.pin_bits(length)
        if count == 1:
            self._entropy.set_text(f"{bits:.1f} bits")
            return
        self._entropy.set_text(f"{count} × {bits:.1f} bits")
    ...
    def _make_phrase(self, *_args: object) -> None:
        ...
        _set_buffer(self._pwd, "\n".join(phrases))
        bits = generate.passphrase_bits(words)
        if count == 1:
            self._entropy.set_text(f"{bits:.1f} bits")
            return
        self._entropy.set_text(f"{count} × {bits:.1f} bits")
    ...
    def _tab_data(self) -> Gtk.Widget:
        ...
        export = common.t_button("data_export")
