...
class DiskPage:
    ...
    def _build(self) -> Gtk.Widget:
        ...
        common.padded(box)
        hint = Gtk.Label(label=i18n.t("disk_hint"), wrap=True, xalign=0)
        hint.add_css_class("dim-label")
        box.append(hint)
        box.append(self._bar.widget)
        self._skip = Gtk.CheckButton(label=i18n.t("disk_skip"))
        self._skip.set_active(True)
        box.append(self._skip)
        self._progress = Gtk.ProgressBar()
        box.append(self._progress)
        actions = Gtk.Box(spacing=8)
        scan = Gtk.Button(label=i18n.t("disk_scan"))
        ...
        parent = Gtk.Button(label=i18n.t("disk_parent"))
        parent.connect("clicked", lambda *_: self._parent())
        export = Gtk.Button(label=i18n.t("disk_export"))
