...
class RenamePage:
    ...
    def _build(self) -> Gtk.Widget:
        ...
        from_folder = Gtk.Button(label=i18n.t("rename_from_folder"))
        from_folder.connect("clicked", lambda *_: self._from_folder())
        sources.append(add)
        sources.append(from_folder)
        box.append(sources)
        hint = Gtk.Label(label=i18n.t("rename_tokens"), wrap=True, xalign=0)
        hint.add_css_class("dim-label")
        box.append(hint)
        self._pat = Gtk.Entry(placeholder_text=i18n.t("rename_pattern"))
        self._pat.set_text("(.*)\\.(\\w+)$")
        self._repl = Gtk.Entry(placeholder_text=i18n.t("rename_repl"))
        self._repl.set_text(r"\1_{n:03d}.\2")
        box.append(self._pat)
        box.append(self._repl)
        affix = Gtk.Box(spacing=8)
        self._prefix = Gtk.Entry(placeholder_text=i18n.t("rename_prefix"))
        self._suffix = Gtk.Entry(placeholder_text=i18n.t("rename_suffix"))
        affix.append(self._prefix)
        affix.append(self._suffix)
        box.append(affix)
        box.append(Gtk.Label(label=i18n.t("rename_case"), xalign=0))
        self._case = compat.string_choice(["—", "lower", "UPPER", "Title", "snake", "kebab"])
        box.append(self._case)
        box.append(Gtk.Label(label=i18n.t("rename_collision"), xalign=0))
        self._collision = compat.string_choice([i18n.t("rename_suffix_n"), i18n.t("rename_skip")])
        box.append(self._collision)
        self._sanitize = Gtk.CheckButton(label=i18n.t("rename_sanitize"))
        box.append(self._sanitize)
        self._overwrite = Gtk.CheckButton(label=i18n.t("rename_overwrite"))
        box.append(self._overwrite)
        self._copies = Gtk.CheckButton(label=i18n.t("rename_copies"))
        box.append(self._copies)
        copy_btn = Gtk.Button(label=i18n.t("rename_out_folder"))
        ...
        self._preset_name = Gtk.Entry(placeholder_text=i18n.t("rename_preset_name"))
        box.append(self._preset_name)
        preset_row = Gtk.Box(spacing=8)
        load_preset = Gtk.Button(label=i18n.t("rename_preset_load"))
        load_preset.connect("clicked", lambda *_: self._load_preset())
        save_preset = Gtk.Button(label=i18n.t("rename_preset_save"))
        save_preset.connect("clicked", lambda *_: self._save_preset())
        import_csv = Gtk.Button(label=i18n.t("rename_csv_import"))
        ...
        prev = Gtk.Button(label=i18n.t("rename_preview"))
        prev.connect("clicked", lambda *_: self._preview())
        apply_btn = Gtk.Button(label=i18n.t("rename_apply"))
        apply_btn.add_css_class("suggested-action")
        apply_btn.connect("clicked", lambda *_: self._apply())
        undo = Gtk.Button(label=i18n.t("rename_undo"))
